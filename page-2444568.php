...
$query = "SELECT p.ID, p.post_title, p.post_name, p.menu_order, t.term_id, t.name " .
  ...
  // fixed by Ilia - need only properties of week with status 'let'
  "INNER JOIN $wpdb->postmeta pm1 ON 
              (pm1.post_id = p.ID) 
              AND (pm1.meta_key='status') 
              AND (pm1.meta_value='let')".
...
<div style="width: calc(100% - 480px);max-width: none;font-size: 31px; color: #ac9857;">New Rental Listings</div>
</div>
