(function ($) {
    ...
    var activeSlide = 0, $activeSlide = $( '#parallax > .content' ).first();
    ...
    $( '#dots li' ).click( function() {
        ...
        // var top = ( ! isFullScreen() ) ? $( div ).offset().top : $( div ).position().top;
        var top = $( div ).offset().top;

        var pos = top - $( '#sub-menu' ).outerHeight() - $( 'nav.top-bar' ).outerHeight();
        $( 'html, body' ).stop( true, false ).animate( { scrollTop: pos }, 1000, 'easeInExpo', function() {
            //is About
...
//		var currentHeight = $content.height();
		    var currentHeight = ( $this.parent().hasClass( 'open' ) ) ? $content.data( 'height' ) : $content.find( '.gform_heading' ).outerHeight() + 6;
...
//        var height = ( $this.parent().hasClass( 'open' ) ) ? $content.data( 'height' ) : 0;
    ...
    function setParallaxContentHeight() {
        ...
        $( '#parallax div.content' ).height( window.innerHeight
...
//        $( '.parallax .shadow' ).css( { top: ( $( '#parallax #home .text' ).offset().top - 60 ) + "px" }  );
    ...
    function showTheSearchButton() {
        ...
        if( activeSlide == $( '#parallax > .content' ).length - 1 ||
          $activeSlide.hasClass( 'no-title' ) ) {
...
//        if( current < 0 ) current = $( '#gallery .images .image' ).length - 1;
    ...
    function centerTheAboutSlides() {
        ...
        $( '.slides .slide-content, .center-me' ).each( function() {
            ...
            var min = $( this ).hasClass( 'text' ) ? 35 : 0;
...
//                    var $padded = $scrollable.find( '> .columns:nth-child(' + ( minColumn + 1 ) + 'n+1)' );
    ...
    function setTheActiveSlide() {
        ...
        $content.each( function( k ) {
            ...
            var minTop = $( window ).scrollTop() + window.innerHeight / 3;
...
//            }
    ...
    $( '#change-view li' ).click( function() {
      ...
      if( $div.data( 'map' ) && ! $div.data( 'initialized' ) ) {
        initGMap( "mapview", $div.data( 'lat' ), $div.data( 'lon' ), true, false );
